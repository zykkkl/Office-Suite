...
1. **封面页**：使用 background_board + 大标题 + 副标题 + 装饰元素 + 动画
2. **内容页**：每页 <= 3 个核心信息点，使用卡片/表格/图表布局
3. **结束页**：深色渐变背景 + 居中文字 + 装饰几何
4. **风格一致**：全篇使用同一 style_preset，颜色、字体保持一致
5. **边界约束**：所有元素 y + height <= 142.875mm，x + width <= 254mm
6. **留白充足**：页面内容占比 <= 70%，避免拥挤
7. **效果丰富**：善用 glow、shadow、gradient、text_effect 提升视觉层次
8. **动画节奏**：标题淡入 → 内容滑入 → 数据缩放，引导观众注意力
9. **图文配合**：元素级图片(type: image)可使用 duotone/grayscale 滤镜统一风格；background_board 中的背景图片不要加滤镜，保持原色以呈现最佳效果
...
5. 每页至少使用 1 种视觉效果（shadow/glow/gradient/border/animation）
